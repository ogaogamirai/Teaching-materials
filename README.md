...
教材制作・改訂時に踏襲すべき設計思想と実践知見のまとめ：
👉 **詳細ドキュメント:** [`PEDAGOGY_GUIDELINES.md`](./PEDAGOGY_GUIDELINES.md)

1. **「結果の数字」ではなく「通常と異なるデータ構造」をじっくり紐解く**: 単に「圧縮率99%」と結果だけ誇るのではなく、過去の履歴ではなく境界（フロンティア）の状態だけを持つような非自明なデータ構造の切り取り方を明かす。
2. **「コマ送り実況」によるブラックボックスの解体**: 4駅などの極小モデルで、ステップごとの状態変化・枝刈り・等価ノード共有の現場を目撃させる。
3. **「トイモデル」から「現実スケール」への二段階跳躍**: 4駅で理屈を腹落ちさせた直後に、87駅などの実社会規模シミュレーターで「本当に現実が動く威力」を体感させる。
4. **数式記号の「音読ルビ」と頭の中の翻訳**: 初出の特殊記号には頭の中での読み方と日常語での意味を徹底併記し、記号への恐怖心をなくす。
5. **認知摩擦の徹底的な削ぎ落とし**: 冒頭のメタ情報を排して即座に本題へ没入させ、本文とシミュレーターの色調統一や堅牢なフォールバックを整える。
